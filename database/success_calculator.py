...
class SuccessCalculator:
    ...
    async def get_success_rate(self, period: str = "last_30_days") -> Dict:
        ...
        try:
            ...
            result = {
                ...
                'success_rate': success_metrics.get('success_rate', 0.0),  # 添加成功率字段
                'profit_rate': success_metrics.get('net_profit', 0.0) / 100000 if len(trades) > 0 else 0.0,  # 添加盈利率
    ...
    async def _calculate_risk_adjusted_metrics(self, 
        ...
        try:
            ...
            if len(returns_array) > 0:
                # 使用布尔索引获取子集
                mask = returns_array <= var_95
                cvar_subset = returns_array[mask]
                cvar_95 = float(np.mean(cvar_subset)) if len(cvar_subset) > 0 else 0.0
            else:
                cvar_95 = 0.0
            
            # 风险回报比
            total_return = sum(returns)
            risk = float(returns_array.std()) if len(returns_array) > 0 else 1.0
            risk_return_ratio = total_return / risk if risk > 0 else 0.0
    ...
    def _calculate_sharpe_ratio(self, trades: List[Dict]) -> float:
        ...
        std_val = returns_array.std()
        if std_val == 0 or np.isclose(std_val, 0):
            return 0.0
        
        # 假设无风险利率为0
        sharpe = (returns_array.mean() / std_val) * np.sqrt(252)
        return float(sharpe)
    ...
    def _calculate_sortino_ratio(self, returns: np.ndarray) -> float:
        ...
        if len(downside_returns) == 0:
            return 100.0  # 替换inf为大数值
        
        downside_std = downside_returns.std()
        if downside_std == 0 or np.isclose(downside_std, 0):
            return 100.0  # 替换inf为大数值
    ...
    def _calculate_calmar_ratio(self, returns: np.ndarray) -> float:
        ...
        if max_dd == 0 or abs(max_dd) < 1e-10:
            return 100.0 if total_return > 0 else 0.0  # 替换inf
    ...
    def _calculate_max_drawdown_from_returns(self, returns) -> float:
        """从收益序列计算最大回撤"""
        if len(returns) == 0:
            return 0.0
